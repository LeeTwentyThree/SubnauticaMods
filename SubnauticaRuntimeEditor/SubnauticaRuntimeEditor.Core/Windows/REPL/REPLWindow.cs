...
using System.Text;
using SubnauticaRuntimeEditor.Core.REPL.MCS;
...
namespace SubnauticaRuntimeEditor.Core.REPL
{
    public sealed class ReplWindow : Window<ReplWindow>
    {
        ...
        private HashSet<string> Namespaces
        {
            get
            {
                if (_namespaces == null)
                {
                    ...
                    SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Debug, $"[REPL] Found {_namespaces.Count} public namespaces");
                }
                return _namespaces;
            }
        }
        ...
        private IEnumerator DelayedReplSetup()
        {
            ...
            {
                SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Warning, "Failed to initialize REPL environment - " + ex.Message);
            }
        }
        ...
        private void RunAutostart(string autostartFilename)
        {
            if (File.Exists(autostartFilename))
            {
                ...
                if (allLines.Length > 0)
                {
                    ...
                    SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Info, message);
        ...
        private void FetchSuggestions(string input)
        {
            ...
            {
                SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Debug, "[REPL] " + ex);
                ClearSuggestions();
            }
        }

...
namespace SubnauticaRuntimeEditor.Core.Profiler
{
    public sealed class ProfilerWindow : Window<ProfilerWindow>
    {
        ...
        protected override void Initialize(InitSettings initSettings)
        {
            ...
            SubnauticaRuntimeEditorCore.PluginObject.StartCoroutine(FrameEndCo());
        }
        ...
        private static void Patch()
        {
                ...
                .Select(t =>
                {
                    if (t.ContainsGenericParameters)
                    {
                        try
                        {
                            ...
                            //SubnauticaRuntimeEditor.Logger.Log(LogLevel.Debug, $"[Profiler] Hooking in generic class {t.FullName} -> {genericType.FullName}");
                            return genericType;
                        }
                        catch (Exception e)
                        {
                            SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Debug, $"[Profiler] Failed to hook in class {t.FullName} - {e.Message}");
                            return null;
                        }
                    }
            ...
            foreach (var hit in hits)
            {
                ...
                {
                    SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Debug, $"[Profiler] Failed to hook {hit.FullDescription()} - {e.Message}");
                }
            }
        }

...
namespace SubnauticaRuntimeEditor.Core.Utils.Abstractions
{
    public static class UnityFeatureHelper
    {
        ...
        static UnityFeatureHelper()
        {
            ...
            if (!SupportsScenes)
                SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Warning, "UnityEngine.SceneManager and/or UnityEngine.SceneManagement.Scene are not available, some features will be disabled");
            ...
            if (!SupportsCursorIndex)
                SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Warning, "TextEditor.cursorIndex and TextEditor.pos are not available, some features will be disabled");
            ...
            if (SupportsCursorIndex)
            {
                ...
                try
                {
                    ...
                    {
                        SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Warning, "Disabling REPL because a profiler is running. This is to prevent the combination of access-modded mcs, profiler and monomod from crashing the process.");
                        SupportsRepl = false;
                    }
                }
                catch (Exception ex)
                {
                    SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Error, ex);
                }
            }
        }

...
namespace SubnauticaRuntimeEditor.Core.Utils.Abstractions
{
            ...
            get
            {
                if (_current == null)
                {
                    try
                    {
                        ...
                        SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Debug, "[UnityInput] Using LegacyInputSystem");
                    }
                    catch (InvalidOperationException)
                    {
                        _current = new NewInputSystem();
                        SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Debug, "[UnityInput] Using NewInputSystem");
                    }
                    catch (Exception ex)
                    {
                        SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Debug, "[UnityInput] Failed to detect available input systems - " + ex);
                    }
                }
                return _current;
            }
    ...
    internal class NewInputSystem : IInputSystem
    {
        ...
        private static ButtonControl GetControl(KeyCode key, bool silent = false)
        {
            ...
            if (!silent)
                SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Debug, $"[{nameof(NewInputSystem)}] Unsupported key: {key}");

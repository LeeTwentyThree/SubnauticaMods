        ...
        public static void DrawButtonIfAvailable()
        {
            if (_abCacheUnloadMethod != null && GUILayout.Button("Clear Bundle Cache"))
            {
                try
                {
                    ...
                    SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Message, "Unloaded " + unloadedCount + " AssetBundles");
                }
                catch (Exception e)
                {
                    SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Message | LogLevel.Error, "Failed to clear the AssetBundle cache - " + e);
                }
            }
        }

...
namespace SubnauticaRuntimeEditor.Core.Utils.Abstractions
{
    public class DnSpyHelper : IFeature
    {
                ...
                if (newValue != _dnSpyPath)
                {
                    ...
                    if (!string.IsNullOrEmpty(_dnSpyPath))
                    {
                        if (File.Exists(_dnSpyPath) && _dnSpyPath.EndsWith("dnspy.exe", StringComparison.OrdinalIgnoreCase))
                            ...
                            SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Error | LogLevel.Message, "[DnSpyHelper] Invalid dnSpy path. The path has to point to 64bit dnSpy.exe");
                    }
        ...
        public static void OpenInDnSpy(ICacheEntry entry)
        {
            ...
            {
                SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Error | LogLevel.Message, "[DnSpyHelper] " + e.Message);
            }
        }
        ...
        private static void StartDnSpy(string args)
        {
            ...
            args = args + " " + DnSpyArgs;
            SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Info, $"[DnSpyHelper] Opening {DnSpyPath} {args}");

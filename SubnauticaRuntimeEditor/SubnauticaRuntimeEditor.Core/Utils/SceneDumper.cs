...
namespace SubnauticaRuntimeEditor.Core.Utils
{
    internal static class SceneDumper
    {
        public static void DumpObjects(params GameObject[] objects)
        {
            ...
            SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Info, $"Dumping {objects.Length} GameObjects to {fname}");
            ...
            SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Info, $"Opening {fname}");
            Process.Start(pi);
        }
        ...
        private static void PrintRecursive(TextWriter sw, GameObject obj, int d = 0)
        {
            ...
            foreach (var c in obj.GetComponents<Component>())
            {
                ...
                foreach (var p in props)
                {
                    ...
                    {
                        SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Debug, e);
                    }
                }
            }

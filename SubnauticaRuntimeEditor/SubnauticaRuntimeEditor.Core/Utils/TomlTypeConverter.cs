...
using UnityEngine;
using ColorUtility = SubnauticaRuntimeEditor.Core.Utils.Abstractions.ColorUtility;
...
namespace SubnauticaRuntimeEditor.Core.Utils
{
    ...
    public static class TomlTypeConverter
    {
        ...
        public static bool AddConverter(Type type, TypeConverter converter)
        {
            ...
            if (CanConvert(type))
            {
                SubnauticaRuntimeEditorCore.Logger.Log(LogLevel.Warning, "Tried to add a TomlConverter when one already exists for type " + type.FullName);
                return false;
            }

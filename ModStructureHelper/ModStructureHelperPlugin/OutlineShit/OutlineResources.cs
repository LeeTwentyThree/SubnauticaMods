...
public sealed class OutlineResources : ScriptableObject
{
    ...
    [FormerlySerializedAs("_renderShader")]
    public Shader renderShader;
    
    [FormerlySerializedAs("_outlineShader")]
    ...
    /// </summary>
    public const string OutlineResourcesTooltip =
        "Outline resources to use (shaders, materials etc). Do not change defaults unless you know what you're doing.";
    
    /// <summary>
    /// Tooltip text for <see cref="OutlineLayerCollection"/> field.
    /// </summary>
    public const string OutlineLayerCollectionTooltip =
        "Collection of outline layers to use. This can be used to share outline settings between multiple cameras.";
    ...
    /// </summary>
    internal const string SrpNotSupported =
        "{0} works with built-in render pipeline only. It does not support SRP (including URP and HDRP).";
    
    /// <summary>
    /// Post-processing not supported message.
    /// </summary>
    internal const string PpNotSupported =
        "{0} does not support Unity Post-processing stack v2. It might not work as expected.";
    ...
    public Material RenderMaterial
    {
        get
        {
            ...
            {
                UnityEngine.Debug.Assert(renderShader != null, "No RenderShader is set in outline resources.", this);
    ...
    public Material OutlineMaterial
    {
        get
        {
            ...
            {
                UnityEngine.Debug.Assert(outlineShader != null, "No OutlineShader is set in outline resources.", this);
    ...
    public Mesh FullscreenTriangleMesh
    {
        ...
        set { _fullscreenTriangleMesh = value; }
    }
    ...
    {
        renderShader = Plugin.AssetBundle.LoadAsset<Shader>("OutlineColor.shader");
        outlineShader = Plugin.AssetBundle.LoadAsset<Shader>("Outline.shader");
    }
    ...
    private void OnValidate()
    {
        if (_renderMaterial)
        {
            _renderMaterial.shader = renderShader;
        }
